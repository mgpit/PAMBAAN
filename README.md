# SimpleKanban – A Simple Kanban Board
## Table of Contents
...
A simple implementation of a KANBAN Board. Tested with Bugzilla v4.2.3. and 5.0.1.

Why develop a Kanban board? There are already several Kanban boards for Bugzilla!

 * I wanted to try programming a Bugzilla Extension
 * Bayoteers provide a Scrum solution (see Agile Tools)
 * Other solutions are based on PHP, Node.js, or whatever else. They are no Bugzilla extensions

Simple means – just simple. No drag'n'drop, no sub laning, no workload enforcement, ...
...
:warning: With version v.0.4.1 the SimpleKanban board internally uses a modified search – so there may be differences between the number of bugs you get when you execute your saved search and when the search is executed for the SimpleKanban board:
